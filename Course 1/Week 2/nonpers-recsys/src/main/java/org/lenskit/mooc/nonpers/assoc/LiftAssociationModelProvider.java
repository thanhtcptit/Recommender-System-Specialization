    ...
    public AssociationModel get() {
        ...
        for (Long2ObjectMap.Entry<LongSortedSet> xEntry: itemUsers.long2ObjectEntrySet()) {
            ...
            for(Long2ObjectMap.Entry<LongSortedSet> y: itemUsers.long2ObjectEntrySet()){
                ...
                long yCount = yUsers.size();
                long xCount = xUsers.size();
                for(long yUser: yUsers){
                    count = (xUsers.contains(yUser) ? count + 1 : count);
                }
                itemScores.put(id,(((float) count * allUsers.size()) / ((float) xCount *  (float) yCount)));
                count = 0;
            }
