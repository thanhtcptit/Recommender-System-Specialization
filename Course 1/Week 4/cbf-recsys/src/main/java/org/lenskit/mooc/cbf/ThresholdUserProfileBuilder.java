    ...
    public Map<String, Double> makeUserProfile(@Nonnull List<Rating> ratings) {
        ...
        for (Rating r : ratings) {
            if (r.getValue() >= RATING_THRESHOLD) {
                Map<String, Double> ItemId = model.getItemVector(r.getItemId());
                for (Map.Entry<String, Double> entry : ItemId.entrySet()) {
                    String tag = entry.getKey();
                    Double minVal = entry.getValue();
                    profile.put(tag, profile.containsKey(tag) ? profile.get(tag) + minVal : minVal);
                }
            }
        }
